#!/usr/bin/env python3
"""Type first-boot config into freebsd13 (node 18) via the QEMU monitor.
...
Run on the EVE-NG host AFTER node 18 has started.

Usage:
  python3 freebsd13_firstboot.py                   # use default port 32786
  python3 freebsd13_firstboot.py --port 36167      # qemu_wrapper direct port
  python3 freebsd13_firstboot.py --wait            # wait 150s for FreeBSD to boot

Note: on some EVE-NG versions the console is on port 32768+node_id (32786)
accessible via the external IP; on others qemu_wrapper opens a random high
port (find it with: ss -tlnp | grep qemu-system).
"""
import socket, time, sys

CONSOLE_PORT = 32786  # default; override with --port <N>
...
def main():
    ...
    print('\n  writing /etc/rc.conf ...')
    m.type_line('sysrc hostname=freebsd13')
    ...
    # VLAN interfaces and per-FIB routes — freebsd13 uses 100.112.1.X subnet
    for vlan, fib, gw_last, host_last, net_last in [
        ...
        gw   = f'100.112.1.{gw_last}'
        host = f'100.112.1.{host_last}'
        net  = f'100.112.1.{net_last}'
    ...
    print('\nDone. freebsd13 rebooting.')
    print('After ~90s, serial console at port 32786 will carry the login prompt.')
